...
def test_done_toggle_marks_row_done(page, live_server):
    ...
    row = page.locator("#tasks-list .triage-item").filter(has_text="Buy milk")
    # Resting state: row not done, toggle shows ✓ but in muted (not done)
    # styling — see .ti-actions .action-btn (no .done class).
    ...
    # Row is now marked done, toggle gets the .done class (green tint),
    # wins counter advanced.
...
def test_done_toggle_can_un_mark(page, live_server):
    ...
    row = page.locator("#tasks-list .triage-item").filter(has_text="Drink water")

    row.locator(".ti-done .action-btn").click()
    expect(page.locator("#wins")).to_contain_text("✓ 1 today")

    # Click again — round-trips back to undone, counter back to 0.
    row.locator(".ti-done .action-btn").click()
    expect(row).not_to_have_class(DONE_CLASS)
...
def test_done_toggle_on_day_block(page, live_server):
    ...
    # done toggle we just added at index 0).
    row = page.locator("#tasks-list .triage-item").filter(
        has_text="Scheduled work"
    )
